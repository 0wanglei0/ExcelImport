...
Popup.prototype={
        ...
        setContent:function(arrt,val){ //设置内容，即 this.info 的参数内容
            if(val!=''){
                switch(arrt){
                    ...
                    case 'contentUrl':thintentUrl=val;break;
                    case 'contentHtml':thintentHtml=val;break;
                    case 'callBack':this.info.callBack=val;break;
                    case 'parameter':this.info.parameter=val;break;
                    case 'confirmCon':thinfirmCon=val;break;
        ...
        build:function(){
            ...
            //关闭按钮
            //var path="";
//            var close='<input type="image" id="dialogBoxClose" src="'+path+'dialogclose.gif" border="0" width="16" height="16" align="absmiddle" title="关闭"/>apos;$;
            var close='<a id="dialogBoxClose" title="关闭" >×</a>';
//          var cB='filter: alpha(opacity='+thiverOpacity+');opacity:'+thiverOpacity/100+';';
            var cover='<div id="dialogBoxBG"></div>';
            var mainBox='<div id="dialogBox" >' + 
            '<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="'+this.color.bColor+'">';
            if(this.config.isHaveTitle) {
                mainBox+='<tr height="24" bgcolor="'+this.color.tColor+'"><td>' +
                '<table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td width="6" height="24"></td>'
            ...
            mainBox+='<tr valign="top"><td id="dialogBody"></td></tr></table></div>'
            ...
            if(this.config.isSupportDraging){
                dropClass=new Dragdrop(this.config.width,this.config.height,this.info.shadowWidth,
                        this.config.isSupportDraging,thintentType);
                G("dialogBoxTitle").style.cursor="move";
            }
            this.lastBuild();
        },
        lastBuild:function() {
            var confirm='<div><div>'+thinfirmCon
            +'</div><div><input id="dialogOk" type="button" value=" 确定 "/>'
            +'<input id="dialogCancel" type="button" value=" 取消 "/></div></div>';
            var alert='<div><div>' + this.info.alertCon
            +'</div><div><input id="dialogYES" type="button" value=" 确定 "/></div></div>';
            var baseZIndex=10001+this.info.overlay*10;var coverIfZIndex=baseZIndex+4;
            if(thintentType==1){
                var openIframe="<iframe width='100%' style='height:"+this.config.height+"px' name='"+this.iframeIdName
                +"' id='"+this.iframeIdName+"' src='"+thintentUrl+"' frameborder='0' scrolling='"
                ...
                var coverIframe="<div id='iframeBG' style='position:absolute;top:0px;left:0px;width:1px;height:1px;z-index:"
                    +coverIfZIndex+'";filter: alpha(opacity=00);opacity:0.00;background-color:#ffffff;><div>';
                G("dialogBody").innerHTML=openIframe+coverIframe
            } else if(thintentType==2){
                G("dialogBody".contentHtml);
            } else if(thintentType==3){
                G("dialogBody").innerHTML=confirm;
                Event.observe(G('dialogOk'),"click",this.forCallback.bindAsEventListener(this),false);
                Event.observe(G('dialogCancel'),"click",this.close.bindAsEventListener(this),false)
            } else if(thintentType==4){
...
//          取得页面的宽度和高度
        ...
        if(typeof(ele)!="undefined"&&ele!=null&&ele.disabled==false){
            ele.disabled=true;this.someToDisabled.push(ele)}},hiddenTag:
                function(tagName){var ele=document.getElementsByTagName(tagName);
                if(ele!=null){for(var i=0;i<ele.length;i++){if(ele[i].style.display!="none"&&ele[i].style.visibility!='hidden'){
                    ele[i].style.visibility='hidden';this.someToHidden.push(ele[i])}}}},
                        ...
                        if(typeof(ele)!="undefined"&&ele!=null){
                            ...
                            showSome:function(){
                                ...
                                for(var i=0;i<this.someToDisabled.length;i++){
                                    this.someToDisabled[i].disabled=false};space("end")}};
var Dragdrop=new Class();
Dragdrop.prototype={
        initialize:function(width,height,shadowWidth,showShadow,contentType){
            this.dragData=null;
            this.dragDataIn=null;this.backData=null;
            this.width=width;this.height=height;this.shadowWidth=shadowWidth;this.showShadow=showShadow;
            this.contentType=contentType;this.IsDraging=false;this.oObj=G('dialogBox');
                ...
                if(this.contentType==1){
                    ...
                    this.backData={
                            ...
                            mousemove:function(event){
                                if(!this.IsDraging)
                                    return;var iLeft=Event.pointerX(event)
                                    -this.dragData["x"]+parseInt(this.oObj.style.left);
                                    ...
                                    if(this.showShadow){
                                        ...
                                        mouseup:function(event){
                                            ...
                                            if(mousX<1||mousY<1||mousX>document.body.clientWidth||mousY>document.body.clientHeight){
                                                this.oObj.style.left=this.backData["x"];this.oObj.style.top=this.backData["y"];
                                                if(this.showShadow){G('dialogBoxShadow').style.left=this.backData.x+this.shadowWidth;
                                                G('dialogBoxShadow').style.top=this.backData.y+this.shadowWidth}};
                                                this.IsDraging=false;document.body.style.cursor="";
